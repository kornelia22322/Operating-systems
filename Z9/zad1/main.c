...
void sig_hanlder(int signo) {
    fprintf(stderr, "Received signal : canceling threads\n");
    for (int p = 0; p < P; p++)
...
void file_handler(char *config_path) {
    FILE *config;
...
void initialize() {
    ...
    //raised when a time interval specified in a call to the alarm or alarmd function expires
    ...
    b_mutex = (pthread_mutex_t*) malloc((N + 2) * sizeof(pthread_mutex_t));
...
//use it as follows : ./main config_file
